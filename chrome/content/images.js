...
/**
 * Provides the image utilities and extensions used by the ImageHandler
 *
 * @namespace ImageHandler
 * @class ImageHandlerChrome.ImageUtils
 */
ImageHandlerChrome.ImageUtils = {
    ...
    updateFileSizeFromCache: function(imageInfo){
        ...
        ImageHandlerChrome.ImageUtils.updateFileSizeFromSyncCache(imageInfo);
        
        if (imageInfo.fileSize <= 0) {
            ImageHandlerChrome.ImageUtils.updateFileSizeFromAsyncCache(imageInfo);
        }
    },
    ...
    updateFileSizeFromSyncCache: function(imageInfo){
        ...
        try {
            ...
            var descriptor = ImageHandlerChrome.httpCacheSession.openCacheEntry(cacheKey, Ci.nsICache.ACCESS_READ, false);
            if (descriptor) {
            ...
            if (ex.result == 0x804B003D) {
                ...
                ImageHandler.Logger.debug("Error: NS_ERROR_CACHE_KEY_NOT_FOUND, cacheKey=" + cacheKey + ", " + imageInfo);
                ...
                ImageHandler.Logger.debug("Error: NS_ERROR_CACHE_WAIT_FOR_VALIDATION, " + imageInfo);
            } else {
                ImageHandler.Logger.warn("Cannot update file size by sync cache for " + imageInfo, ex);
            }
        }
        ...
        if (fileSize > 0) {
            ...
            ImageHandler.Logger.info("update file size to " + fileSize + " from sync cache for " + imageInfo);
        }
    },
    ...
    updateFileSizeFromAsyncCache: function(imageInfo){
        ...
        ImageHandler.Logger.info(" try to update file size by async cache for " + imageInfo);
        
        // try to update file size by cache listener
        var listener = new ImageHandlerChrome.CacheListener(imageInfo);
        try {
            ImageHandlerChrome.httpCacheSession.asyncOpenCacheEntry(cacheKey, Ci.nsICache.ACCESS_READ, listener);
        } catch (ecache) {
            ImageHandler.Logger.warn("Cannot update file size by async Cache for " + imageInfo, ecache);
        }
    },
    ...
    updateFileNameFromCache: function(imageInfo){
        ...
        try {
            ...
            ImageHandler.Logger.debug("find content props = " + props + " for " + imageInfo);
            if (props) {
                ...
                if (props.has("type")) {
                    ...
                    if (contentType) {
                        ...
                        if(mime != null && !mime.extensionExists(imageInfo.fileExt)){
                            ...
                            ImageHandler.Logger.info("update file ext to " + mime.primaryExtension + " from content-type for " + imageInfo);
                        }
                   }
                }
                ImageHandler.Logger.debug("contentDisposition = " + contentDisposition + ", contentType = " + contentType + " for " + imageInfo);
            }
        } catch (e) {
            //Ignore
            ImageHandler.Logger.warn("Failure to get type and content-disposition of the image " + imageInfo, e);
        }
        ...
        if (contentDisposition) {
            ...
            if (fileName != null) {
                ...
                ImageHandler.Logger.info("update file name to " + fileName + " from content-disposition for " + imageInfo);
            }
        }
    },
    ...
    updateFileExtensionByMIME: function(imageInfo){
        ...
        try {
            ...
            if(mime != null && !mime.extensionExists(imageInfo.fileExt)){
                imageInfo.setFileExt(mime.primaryExtension);
                ImageHandler.Logger.info("update file extension to " + mime.primaryExtension + " by MIME for image:" + imageInfo);
            }
        } catch (e) {
            ImageHandler.Logger.info("cannot update file extension by MIME for image" + imageInfo + ", e = " + e);
        }
    },
 ...
 *
 * @namespace ImageHandler
 * @class ImageHandlerChrome.CacheListener
 * @constructor
 * @param {ImageInfo}
 *            image info object to update file size
 */
ImageHandlerChrome.CacheListener = function(imageInfo){
    this.imageInfo = imageInfo;
};

ImageHandlerChrome.CacheListener.prototype = {
    ...
    accessGranted, /* in nsresult */ status){
        ...
        if ((fileSize && (fileSize > 0)) && ((this.imageInfo.fileSize == null) || (this.imageInfo.fileSize == 0))) {
            ...
            ImageHandler.Logger.info("update file size to " + fileSize + " by async cache for " +
            this.imageInfo);
        } else {
            ImageHandler.Logger.warn("Ingore file size: " + fileSize + " for " + this.imageInfo);
        }
    }
};